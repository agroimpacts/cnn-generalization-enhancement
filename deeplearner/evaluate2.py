...
class Evaluator(object):
    ...
    def _generate_matrix(self, ref_img, pred_img):
        ...
        where each row corresponds to a class in the ground truth, and each 
        column corresponds to a class in the prediction. The (i, j) element of 
        the matrix is the number of pixels that belong to class i in the ground 
        truth and are classified as class j in the prediction.
...
#     def plot_confusion_matrix(self, class_mapping, unknown_class_idx=None, save_path="confusion_matrix.png"):
#         # check for ignore_index and create a mask acordingly
#         if unknown_class_idx is not None and 0 <= unknown_class_idx < self.num_class:
#             mask = np.ones(self.num_class, dtype=bool)
#             mask[unknown_class_idx] = False
#             conf_mat_filtered = self.confusion_matrix[mask][:, mask]
#         else:
#             conf_mat_filtered = self.confusion_matrix
        
#         # Normalize the confusion matrix by row (i.e., by the true class)
#         row_sums = conf_mat_filtered.sum(axis=1, keepdims=True)
#         conf_mat_normalized = np.divide(conf_mat_filtered, row_sums, where=row_sums!=0)
        
#         # Adjust the class labels to exclude the ignored class
#         if unknown_class_idx is not None and 0 <= unknown_class_idx < self.num_class:
#             classes = [class_mapping[i] for i in range(self.num_class) if i != unknown_class_idx]
#         else:
...
#     def plot_confusion_matrix(self, class_mapping, 
...
#         conf_mat_normalized = np.divide(conf_mat, row_sums, where=row_sums!=0)
...
#         # Use seaborn to plot the heatmap
#         heatmap = sns.heatmap(df_cm, annot=True, fmt=".3f", cmap='viridis', 
...
#         plt.show()
    ...
    def reset(self):
        ...
        start a new round of evaluation. It can be useful in cases where 
        evaluation is done in an episodic manner, such as when evaluating model 
        performance after each epoch during training.
        """
        self.confusion_matrix = np.zeros((self.num_class,) * 2)


def evaluate(model, dataloader, num_classes, class_mapping, device, 
             unknown_class_idx=None, buffer=None, out_name=None):
    """
    Evaluate the performance of a trained model on a dataset and calculate 
    various metrics.
    
    Args:
        model (torch.nn.Module): The trained model to be evaluated.
        dataloader (torch.utils.data.DataLoader): The dataloader for the 
            evaluation dataset.
        num_classes (int): The number of target classes in the dataset.
        device (torch device): whether to map the model on cpu or gpu.
            device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        unknown_class_idx (int): Class index for the 'nodata' or 'unknown' class to be 
                                 excluded from metric calculation.
        class_mapping (dict): A dictionary mapping class indices to class names.
        buffer (int, optional): The number of pixels to ignore along the 
            boundary of the images in the batch. If None, no pixels are ignored. 
            Defaults to None.
        out_name (str, optional): The evaluation metrics filename. If None, the generic 
                                  name 'metrics.csv' will be used. The path is handled 
                                  when we call the function within compiler. Defaults to None.
    
    Returns:
        dict: A dictionary containing the calculated metrics including Overall 
            Accuracy, Mean Accuracy, Mean IoU (Intersection over Union), 
            mean Precision, and mean Recall.
            
    Notes:
        - The function calculates confusion matrix and plots it using seaborn. 
        - Function generates both class-wise and overal mean metrics and saves 
          them into two separate CSV files.
        - CSV files are saved in the 'working_directory / out_dir'.
    """
    ...
    if out_name:
        ...
        evaluator.plot_confusion_matrix(class_mapping, unknown_class_idx, plot_name)
    else: 
